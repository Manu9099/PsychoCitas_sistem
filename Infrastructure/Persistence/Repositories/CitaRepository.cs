...
{
    public async Task<IEnumerable<Cita>> GetAgendaDiaAsync(Guid psicologoId, DateOnly fecha, CancellationToken ct = default)
        => await _dbSet
            .Include(c => c.Paciente)
            .Include(c => c.Nota)
            .Include(c => c.Pago)
            .Where(c => c.PsicologoId == psicologoId
                     && DateOnly.FromDateTime(c.FechaInicio) == fecha)
            .OrderBy(c => c.FechaInicio)
            .ToListAsync(ct);

    public async Task<IEnumerable<Cita>> GetByPacienteAsync(Guid pacienteId, CancellationToken ct = default)
        => await _dbSet
            .Include(c => c.Pago)
    ...
    public async Task<bool> ExisteSolapamientoAsync(
        Guid psicologoId, DateTime inicio, DateTime fin,
        Guid? excluirId = null, CancellationToken ct = default)
    ...
    public async Task<IEnumerable<Cita>> GetProximasConRecordatorioAsync(
